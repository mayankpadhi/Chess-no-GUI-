...
public class queen extends pieces{
    ...
    public boolean chkmoves(int frmposx, int frmposy, int toposx, int toposy, boolean color){
        ...
        int flag= 0;
        while(i!= 8 || j!= 8){
            //if a piece is in the way of the queen and final position
            if(brd.board[i][j]!= null && i<toposx && j< toposy){
                flag= 0;
                return false;
            }
            
            //if final position is reached and is empty
            else if(brd.board[toposx][toposy]== null && brd.board[toposx][toposy]== brd.board[i][j]){
                return true;
            }
            
            //if final position is reached but not empty
            else if(brd.board[toposx][toposy]== brd.board[i][j]){
                
                //if final position has piece of same color
                if(brd.board[toposx][toposy].col== color){
                    flag= 0;
                    return false;
                }
                
                //if final position has oppenents piece
